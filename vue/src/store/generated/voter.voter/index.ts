	...
	actions: {
		...
		async QueryPoll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.VoterVoter.query.queryPoll( key.id)).data
		...
		async QueryPollAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.VoterVoter.query.queryPollAll(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.VoterVoter.query.queryPollAll({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgDeletePoll({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.VoterVoter.tx.sendMsgDeletePoll({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async sendMsgUpdatePoll({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.VoterVoter.tx.sendMsgUpdatePoll({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgDeleteVote({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.VoterVoter.tx.sendMsgDeleteVote({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgDeletePoll({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.VoterVoter.tx.msgDeletePoll({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgDeletePoll:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgUpdatePoll({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.VoterVoter.tx.msgUpdatePoll({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgUpdatePoll:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgDeleteVote({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.VoterVoter.tx.msgDeleteVote({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgDeleteVote:Init Could not initialize signing client. Wallet is required.')
